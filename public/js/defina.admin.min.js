new Vue({
    ...
    methods: {
        ...
        async getImageUuid(uuid)
        {
            var tryimage = document.querySelectorAll('.tryimage')
            tryimage.forEach((item) => {
                item.src = '/img/placeholder.png';
            });
            ...
            const res = await fetch(`/api/image/${uuid}`, options)
            ...
            promise.then((value) => {
                ...
                tryimage.forEach((item, i) => {
                    console.log('Image:', i, item)
                    item.src += value;
                });
            });
            //console.log('Image:', res)
            // return data ? data : '/img/placeholder.png'
        },
        async editText(uuid)
        {
            this.loading = false
...
var loaderphoto = document.getElementById("loaderphoto");
if(loaderphoto) {
    var image = document.getElementById("isEmptyImage").getAttribute('src');
    if(!image) {
        document.getElementById("isEmptyImage").src = '/img/placeholder.png';
    }
...
function loadPfoto() {
    ...
    if(x !== '') {
        document.getElementById("loaderphoto").submit();
        document.getElementById("isloader").innerHTML = '<span class="material-symbols-outlined spin" v-else>autorenew</span> Загружаем фото...';
    }
    //document.getElementById("isloader").innerHTML = '';
}

var successphoto = document.getElementById('successphoto');
if(successphoto) {
    toastr.success(successphoto.innerText, "Успешно", {
        positionClass:"toast-bottom-left",
        containerId:"toast-bottom-left"
    });
}
