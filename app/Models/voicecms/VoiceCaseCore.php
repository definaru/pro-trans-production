...
use App\Models\voicecms\VoiceDefina;
use App\Models\voicecms\VoiceDefinaModul;

/**
 * Набор основных функций, который позволяют сделать интерфейс слонения русского языка
 * Содержит все функции для внешнего взаимодействия с библиотекой.
 * 
 * @author Defina LLC
 */
class VoiceCaseCore extends Voice
{   
    ...
    /**
     * Массив содержит елементы типа VoiceDefinaModul. Это все слова которые нужно обработать и просклонять
     * @var array
     */
    private $words = array();
    /**
    ...
    protected function Last($length=1, $stopAfter=0)
    {
        ...
        {
            $this->workindLastCache[$length][$stopAfter] = VoiceDefina::substr($this->workingWord, -$length, $cut);
        }
    ...
    protected function in($letter, $string)
    {
        ...
        {
            if (!$letter or VoiceDefina::strpos($string, $letter) === false)
            {
                return false;
            }
            else
    ...
    protected function inNames($nameNeedle, $names)
    {
        ...
        {
            if (VoiceDefina::strtolower($nameNeedle) == VoiceDefina::strtolower($name))
            {
                return true;
            }
        }
    ...
    protected function wordForms($word, $endings, $replaceLast=0)
    {
        ...
        $word = VoiceDefina::substr($word, 0, VoiceDefina::strlen($word) - $replaceLast);
        //Добавляем окончания
    ...
    /**
     * В массив <var>$this->words</var> добавляется новый об’єкт класса VoiceDefinaModul
    ...
    public function setFirstName($firstname="")
    {
        if ($firstname)
        {
            ...
            $this->words[$index] = new VoiceDefinaModul($firstname);
    ...
    public function setSecondName($secondname="")
    {
        ...
        {
            $index = count($this->words);
            $this->words[$index] = new VoiceDefinaModul($secondname);
    ...
    public function setFatherName($fathername="")
    {
        ...
        {
            $index = count($this->words);
            $this->words[$index] = new VoiceDefinaModul($fathername);
     ...
     * @param VoiceDefinaModul $word слово которое нужно идентифицировать
     */
    private function prepareNamePart(VoiceDefinaModul $word)
    {
        if (!$word->getNamePart())
        {
     ...
     * @param VoiceDefinaModul $word слово для которого нужно определить пол
     */
    private function prepareGender(VoiceDefinaModul $word)
    {
    ...
    private function solveGender()
    {
        ...
        foreach ($this->words as $word)
        {
            ...
            $man+=$gender[Voice::$MAN];
            $woman+=$gender[Voice::$WOMAN];
        }
        if ($man > $woman)
        {
            $this->setGender(Voice::$MAN);
        }
        else
        {
            $this->setGender(Voice::$WOMAN);
        }
        return true;
    }
    /**
    ...
    private function splitFullName($fullname)
    {
        ...
        {
            $this->words[] = new VoiceDefinaModul($word);
        }
        $this->prepareEverything();
        $formatArr = array();
        foreach ($this->words as $word)
     ...
     * @param VoiceDefinaModul $word слово, которое нужно просклонять
     */
    private function WordCase(VoiceDefinaModul $word)
    {
        $gender = ($word->gender() == Voice::$MAN ? 'man' : 'woman');
        $namepart = '';
     ...
     * @param VoiceDefinaModul $word слово для котрого нужно вернуть падеж
     * @param int $number номер падежа, который нужно вернуть
     * @return mixed массив или строка с нужным падежом 
     */
    private function getWordCase(VoiceDefinaModul $word, $number=null)
    {
    ...
    public function getFormattedArray($format)
    {
        ...
        $length = VoiceDefina::strlen($format);
        $result = array();
        $cases = array();
        ...
        for ($curCase = 0; $curCase < $this->CaseCount; $curCase++)
        {
            ...
            for ($i = 0; $i < $length; $i++)
            {
                $symbol = VoiceDefina::substr($format, $i, 1);
                if ($symbol == 'S')
                {
    ...
    /**
     * Возвращает внутренний массив $this->words каждая запись имеет тип VoiceDefinaModul
     ...
     * @param VoiceDefinaModul $word обьект класса слов, для которого нужно определить пол
     */
    protected function GenderByFirstName(VoiceDefinaModul $word)
    {
     ...
     * @param VoiceDefinaModul $word обьект класса слов, для которого нужно определить пол
     */
    protected function GenderBySecondName(VoiceDefinaModul $word)
    {
     ...
     * @param VoiceDefinaModul $word обьект класса слов, для которого нужно определить пол
     */
    protected function GenderByFatherName(VoiceDefinaModul $word)
    {
     ...
     * - <b>S</b> - фамилия
     * - <b>F</b> - отчество
     * @param VoiceDefinaModul $word обьект класса слов, который необходимо идентифицировать
     */
    protected function detectNamePart(VoiceDefinaModul $word)
    {
