...
/**
 * класс, который служит для хранения всей информации о каждом слове of VoiceDefinaModul
 * 
 * @author Defina LLC
 */
class VoiceDefinaModul 
{
    /**
     ...
     * - 0 - не определено
     * - Voice::$MAN - мужской пол
     * - Voice::$WOMAN - женский пол
     * @var int 
     */
    ...
    public function __construct($word)
    {
        ...
        $this->word = VoiceDefina::strtolower($word);
    }
    ...
    {
        $letters = VoiceDefina::splitLetters($word);
        $mask = array();
        $this->isUpperCase = true;
        foreach ($letters as $letter)
        {
            if (VoiceDefina::isLowerCase($letter))
            {
                $mask[] = 'x';
    ...
    private function returnMask()
    {
        if ($this->isUpperCase)
        {
            ...
            {
                $this->NameCases[$index] = VoiceDefina::strtoupper($this->NameCases[$index]);
            }
        }
        else
        {
            ...
            foreach ($this->NameCases as $index => $case)
            {
                $caseLength = VoiceDefina::strlen($case);
                $max = min(array($caseLength, $maskLength));
                $this->NameCases[$index] = '';
                for ($letterIndex = 0; $letterIndex < $max; $letterIndex++)
                {
                    $letter = VoiceDefina::substr($case, $letterIndex, 1);
                    if ($splitedMask[$letterIndex] == 'X')
                    {
                        $letter = VoiceDefina::strtoupper($letter);
                    }
                    $this->NameCases[$index] .= $letter;
                }
                $this->NameCases[$index] .= VoiceDefina::substr($case, $max, $caseLength-$maskLength); 
            }
        }
    }
    ...
    public function gender()
    {
        if (!$this->genderSolved)
        {
            ...
            {
                $this->genderSolved = Voice::$MAN;
            }
            else
            {
                $this->genderSolved = Voice::$WOMAN;
            }
        }
    ...
    {
        return array(Voice::$MAN => $this->genderMan, Voice::$WOMAN => $this->genderWoman);
    }
